...
traffic_sign_model = cv2.dnn.readNetFromONNX(
    r"C:\Users\monsi\Downloads\hello-via\p2_traffic_sign_detection\traffic_sign_classifier_lenet_v3.onnx")
...
# Initialize CarLogic instance globally
car = CarLogic()
...
async def process_image(websocket, path):
    async for message in websocket:
        ...
        draw = image.copy()
        # Detect traffic signs
        signs = detect_traffic_signs(image, traffic_sign_model)
        # Send back throttle and steering angle
        throttle, steering_angle = car.decision_control(image, signs, draw)
